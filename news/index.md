...
- [`list_videocats()`](https://gojiplus.github.io/tuber/reference/list_videocats.md)
  ...
  Note for
  [`list_guidecats()`](https://gojiplus.github.io/tuber/reference/list_guidecats.md)
  specifically: Google deprecated the `guideCategories` endpoint in
  September 2020 and it no longer returns data. The request is now
  assembled correctly, but the method it targets is gone, so this
  repairs the client rather than restoring the feature.

- [`get_live_streams()`](https://gojiplus.github.io/tuber/reference/get_live_streams.md)
  accepted `channel_id` and required either that or `stream_id`, then
  sent `channelId` alongside `broadcastStatus`. `liveBroadcasts.list`
  has no `channelId` parameter and accepts exactly one of
  `broadcastStatus`, `id` or `mine`, so a status-only query was
...
- **NEW ENDPOINTS**: Support for previously missing YouTube API
  endpoints:
  - [`get_live_streams()`](https://gojiplus.github.io/tuber/reference/get_live_streams.md):
    Live broadcast and premiere information
...
- **PAGINATION**: Fixed
  ...
  pagination bug where comments were duplicated instead of fetching
  unique results when `max_results > 100` (issue
...
- **PLAYLIST ITEMS**: Fixed
  ...
  returning only metadata instead of actual video items (issue
  [\#95](https://github.com/gojiplus/tuber/issues/95))
...
- [`list_channel_resources()`](https://gojiplus.github.io/tuber/reference/list_channel_resources.md):
  ~95% reliability improvement with retry logic
