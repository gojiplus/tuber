...
comments <- comments |>
  mutate(
    has_emoji = has_emoji(text_display),
    emoji_count = count_emojis(text_display)
  )
...
all_emojis <- unlist(extract_emojis(comments$text_display))
...
comments <- comments |>
  mutate(
    emojis = extract_emojis(text_display),
...
ggplot(comments, aes(x = has_emoji, y = like_count + 1)) +
...
video_emoji_stats <- all_comments |>
  mutate(emoji_count = count_emojis(text_display)) |>
  group_by(video_id) |>
  summarise(
    total_comments = n(),
...
comments <- comments |>
  mutate(
    clean_text = remove_emojis(text_display),
