...
The latest development version of the package will always be on GitHub.
...
To get going, get the application id and password from Google Developer
Console (see
<https://developers.google.com/youtube/v3/getting-started>). Enable
YouTube APIs. Create OAuth credentials, being sure to select ‘Other’ as
your Application Type. Then set the application id and password via the
...
get_video_stats(video_ids = "N708P-A45D0")
```
...
get_video_details(video_ids = "N708P-A45D0")
```
...
tracks <- list_captions(video_id = "yJXTXN4xrI8")
download_caption(tracks$caption_id[[1]])
```
...
res <- list_comment_threads(c(video_id = "N708P-A45D0"))
head(res)
```
...
a <- get_channel_details(
  channel_ids = "UCT5Cx1l4IS3wHkJXNyuj4TA",
  part = "contentDetails",
  simplify = FALSE
)
...
vids <- list_playlist_items(playlist_id = playlist_id)

# Video ids
vid_ids <- vids$video_id

# Get stats for all videos
res_df <- get_video_stats(vid_ids)

head(res_df)
```

If you need to find a channel ID from a legacy `username`,
[`get_channel_details()`](https://gojiplus.github.io/tuber/reference/get_channel_details.md)
can help:

``` r

res <- get_channel_details(usernames = "GoogleDevelopers", part = "id")
